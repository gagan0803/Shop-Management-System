...
db=mysql.connector.connect(user="root",passwd="gagan@2003",host="localhost") 
...
db=mysql.connector.connect(user="root",passwd="gagan@2003",host="localhost",database='Shop') 
my_cursor=db.cursor()
...
def bill():
    ...
    # Initialize list to store sale details for bulk insertion
    sale_details = []
    
    # Checking if the quantity of each product is entered, calculating price, showing it on window, and adding to sale details list
    if len(name1.get()) != 0:
        i = res[0]
        qty = int(name1.get())
        total = qty * int(i[2])
        ...
        totalBill += total
        y += 0.1
        sale_details.append((cName, dt, i[1], qty, total))
    ...
    # Bulk insert sale details into database
    try:
        query = "INSERT INTO sale(custName, date, prodName, qty, price) VALUES (%s, %s, %s, %s, %s)"
        cursor.executemany(query, sale_details)
        db.commit()
        messagebox.showinfo('Success', 'Bill generated and sale details uploaded successfully')
    except Exception as e:
        print("The exception is:", e)
        messagebox.showinfo("Error", "Failed to upload sale details to database")
